...
class _OrdersClass extends State<OrdersClass> {
  ...
  Widget getContainerForPrice()
  {
    return Column(
      ...
      children: [
        GestureDetector(
          onTap: (){
            checkOrderValidity(vendorData['userid'], orderData['orderid'], orderData['id']).whenComplete(() {
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (context) => HomeDashbord(vendorData),
  ...
  Future<void> checkOrderValidity(String vendorid,String orderid,oid)
  async {
    String graphQLDocument = '''
      mutation MyMutation {
      updateOrders(input: {status: "confirm", id: "$oid", takenbyid: "$vendorid"}, condition: {status: {eq: "waiting"}, takenbyid: {eq: "no one"}, orderid: {eq: "$orderid"}}) {
        activation
        address
        cartype
        createdAt
