...
class _HomeDashbord extends State<HomeDashbord> {
  ...
  void initState() {
    ...
    checkAprovel();
    // getMyAppointmentList(data['userid']).then((value){
    //   print("test mode on");
    //   print(value.length);
    //
    //   setState(() {
    ...
    get_data();
//     oc.getData(data['id'], data['mobileno']).then((service){
//       print("service");
//       print(service);
//       print(data['id'] + data['mobileno']);
//       oc.showOrders(data['id'], data["mobileno"],double.parse(data['lat']),double.parse(data['log'])).then((orders){
//
//
//
//
...
//             {
//               if(service[i]['cartype'] == orders[j]['cartype'] && service[i]['servicename'] == orders[j]['servicename'])
//               {
//                   rectifydata.add(orders[j]);
//                   print("ghghgh");
//                   print(rectifydata);
...
//         // {
//         // if(service[j]['servicename'] == orders[j]['servicename'] && data[i]['cartype'] == orders[j]['cartype'])
//         // {
...
//     });
  ...
  void updateUserProfile() {
    scaffoldkey.currentState.showBottomSheet((context) {
      return NewBottomSheet(data['mobileno']);
    }).closed.whenComplete(() {
      FetchUserData fud = FetchUserData();
      fud.fetchData(data['mobileno']).then((value) {
        setState(() {
          this.data = value;
        });
      });
        ...
        actions: <Widget>[
          CupertinoActionSheetAction(
            ...
            onPressed: () {
              ...
              deleteServiceData(id, mobileno, serviceid).whenComplete((){
                setState(() {
                  fms.getData(this.data['userid'], this.data['mobileno']).then((value) {
                    service_i_have.clear();
                    setState(() {
  ...
  Future<List> getMyAppointmentList(String vendorid)
  async {
    String graphQLDocument = '''
      query MyQuery {
      listOrderss(filter: {takenbyid: {eq: "$vendorid"}}) {
        items {
          activation
          address
          cartype
...
Future<void> deleteServiceData(String id,String mobileno,String serviceid)
async {
  print("id = $id");
  print("mob = $mobileno");
  print("serviceid = $serviceid");
    String graphQLDocument = '''
    mutation MyMutation {
      deleteService(input: {id: "$id"}, condition: {serviceid: {eq: "$serviceid"}, mobileno: {eq: "$mobileno"}}) {
        serviceid
        }
      }
  ...
  void updateUserData(String username,String email)
  {
    if(_image != null)
    {
      ...
      uia.uploadProfileImage(_image).then((value){
        ...
        updateUserProfileData(username, email, data['id'], mobileno, value, 1);

      });
    }
    else {
      updateUserProfileData(username,email,data['id'],mobileno,null,0);
    }
  }

  void updateUserProfileData(String name,String mail,String id,String mobileno,String profile,int w)
  async{
    String graphQLDocument = "";
    if(w == 0) {
      graphQLDocument  = '''
      mutation MyMutation {
        updateUserData(input: {name: "$name", mail: "$mail", id: "$id"}, condition: {mobileno: {eq: "$mobileno"}}) {
          name
          mail
        }
      }
    ''';
      ...
      graphQLDocument  = '''
      mutation MyMutation {
        updateUserData(input: {name: "$name", mail: "$mail",profile: "$profile", id: "$id"}, condition: {mobileno: {eq: "$mobileno"}}) {
          name
          mail
          profile
        }
